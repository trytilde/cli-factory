...
.PHONY: test test-unit test-e2e test-provider test-provider-tool ensure-docs-submodule generate-docs generate-catalog build build-all sops-encrypt sops-decrypt sops-encrypt-provider-test-secrets help
...
test-unit: ## Run internal unit tests
	go test ./internal/...

test-e2e: ## Run all provider e2e tests
	go test ./providers/... -run TestE2E

test-provider: ## Run e2e tests for one provider: make test-provider PROVIDER=google
	@test -n "$(PROVIDER)" || (echo "PROVIDER is required" && exit 2)
	go test ./providers/$(PROVIDER)/... -run TestE2E

test-provider-tool: ## Run e2e tests for one tool: make test-provider-tool PROVIDER=google TOOL=send-email
	@test -n "$(PROVIDER)" || (echo "PROVIDER is required" && exit 2)
...
generate-docs: ensure-docs-submodule ## Generate Mintlify provider/tool docs from metadata
	go run ./tools/generatedocs --docs-root $(DOCS_REPO_DIR)

generate-catalog: ## Generate embedded tool catalogue and embeddings when OPENAI_API_KEY is available
...
build: ## Build local factory binary
	CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o dist/factory ./cmd/factory

build-all: ## Build static binaries for Linux, macOS, and Windows
	@mkdir -p dist
